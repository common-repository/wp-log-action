...
class WPLA_Logger {
    ...
    private function __construct() {
        //now that table_name is not here, no real purpose for singleton.
    }

    /**
     * Sets up the table_name according to the current wpdb prefix.
     */
    public function get_table_name() {
        global $wpdb;
        return $wpdb->prefix . 'wpla_logs';
    }
     ...
     */
    public function get_results( $args, $type = OBJECT ) {
        global $wpdb;
        $r = wp_parse_args( $args, array(
            's'              => '', //search
            'log_type'       => '',
        ...
        if ( !empty( $r['paged'] ) && !empty( $items_per_page ) && $items_per_page != -1 ) {
        ...
        return array( 
            ...
            "per_page" => $items_per_page != -1 ? $items_per_page : $total_items,
            'results' => $wpdb->get_results( "select * " . $query, $type )
        );
    }
     ...
     */
    public function to_csv( $query_args ) {
        $filename = apply_filters( 'wpla_csv_filename', 'wp-log-action-logs.csv' );
        $results = $this->get_results( $query_args, ARRAY_A );
        $result = $results['results'];
        if ( empty( $result ) ) {
            $filename = 'NO_RESULTS_' . $filename;
        }
        ...
        if( !empty( $result ) ) {
            ...
            $row = current( $result );
            if( $row ) {
                fputcsv( $fp, array_map( 'strtolower', array_keys( $row ) ) );
            }
        }
        ...
        foreach ($result as $row ) {
            ...
            foreach( $row as $key => $value ) {
                $filtered_row[$key] = str_replace( array( "\n", "\r" ), '', $value );
            }
            fputcsv( $fp, $row );
        }

        if ( empty( $result ) ) {
            fputcsv( $fp, array( 'NO_RESULTS' ) );
        }
